...
class IndexLikeCommentTest extends TestCase
{
    use RefreshDatabase;

    public function test_index_like_comment(): void
    {
        ...
        $like_1 = Like::factory()->create(
            ['likeable_id' => $comment->id, 'likeable_type' => Comment::class, 'user_id' => $user->id]
        );
        ...
        $response = $this->getJson("/api/v1/users/{$user->id}/posts/{$post->id}/comments/{$comment->id}/likes");
